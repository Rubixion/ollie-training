...
export function Entropy({ className = "", size = 400 }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    const gridSize = 25; const spacing = size / gridSize
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        const x = spacing * i + spacing / 2; const y = spacing * j + spacing / 2
        particles.push(new Particle(x, y, x < size / 2))
      }
    }
    function updateNeighbors() {
      particles.forEach(particle => {
        particle.neighbors = particles.filter(other => other !== particle && Math.hypot(particle.x - other.x, particle.y - other.y) < 100)
      })
    }
    let time = 0; let animationId = 0
    function animate() {
      ctx.clearRect(0, 0, size, size)
      if (time % 30 === 0) updateNeighbors()
      particles.forEach(particle => {
        particle.update(); particle.draw(ctx)
      ...
      ctx.strokeStyle = `${particleColor}4D`; ctx.lineWidth = 0.5
      ctx.beginPath(); ctx.moveTo(size / 2, 0); ctx.lineTo(size / 2, size); ctx.stroke()
      time++; animationId = requestAnimationFrame(animate)
    }
    animate()
